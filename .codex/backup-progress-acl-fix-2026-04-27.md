...
- 已完成：优化专项 `O1` 公共页取数减重 + 缓存策略收口，目标页为 `/`、`/home`、`/featured`、`/discussions`。
- 已完成：优化专项 `O2` 热点 SQL / `EXPLAIN ANALYZE` / 缺失索引补齐，优先覆盖 `/api/feed/home`、`/api/discussions/home`、`/api/prompts` 与详情页相关查询。
- 进行中：优化专项 `O3` 媒体链路稳定化，已完成“上传大小护栏 + 媒体回调写回 + 视频提交真实异步任务 ID + 资源用途/对象键契约收口 + 共享读链路头像资产兼容 + 本地媒体对象键审计”，剩余重点为真实 OSS provider、派生媒体异步生成与业务表对资产引用的进一步收口。
- 已完成：优化专项 `O4` 稳定性基建第一阶段，已补齐后端 HTTP 集成测试、默认本地稳定性套件、浏览器/API smoke、本地过期数据清理、启动后验活 / 关键依赖自检；后续如果继续扩展，重点转到测试环境级联自检和云资源联调口径。
- 进行中：优化专项 `O5` 前端体感速度优化，现已完成“公共页次要 prompt 区块短超时 + 软降级 + `/home` hero 主 feed 回退 + `/` `/home` 路由层只等主 feed + `/featured` 主内容先出、补充库存后到”三刀，下一步再看是否需要把首页 `/` 的非关键辅助区块也拆成更独立的次级渲染边界。
- 待用户提供本地资源路径后推进：`P2` 首页 `/` 与精选 `/featured` 去 demo 资源，替换为真实素材。
- 待推进：`P3` 举报、通知、个人设置/资料编辑从前端占位切到真实后端能力。
- 待推进：`P4` 评论增强剩余项，包括评论举报、媒体插入、更细的审核处置与通知联动；`楼中楼 / 评论治理（开关、删除）` 已完成第一版落地。
