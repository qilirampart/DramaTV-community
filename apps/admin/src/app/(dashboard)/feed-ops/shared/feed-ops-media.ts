...
const ADMIN_MEDIA_BASE_URL =
  process.env.DRAMATV_ADMIN_API_BASE_URL?.trim() ||
  process.env.NEXT_PUBLIC_DRAMATV_ADMIN_API_BASE_URL?.trim() ||
  "http://127.0.0.1:18080";
const WEB_MEDIA_BASE_URL =
  process.env.DRAMATV_WEB_BASE_URL?.trim() ||
  process.env.NEXT_PUBLIC_DRAMATV_WEB_BASE_URL?.trim() ||
  "http://127.0.0.1:3106";
const ADMIN_PUBLIC_BASE_PATH = normalizeBasePath(
  process.env.NEXT_PUBLIC_DRAMATV_ADMIN_BASE_PATH?.trim() || "/admin"
);
const WEB_STATIC_MEDIA_PREFIXES = ["/seedance-videos/", "/nano-banana-images/"];
const ADMIN_SAME_ORIGIN_PROXY_PREFIX = ADMIN_PUBLIC_BASE_PATH ? `${ADMIN_PUBLIC_BASE_PATH}/__admin_proxy__` : "/__admin_proxy__";
